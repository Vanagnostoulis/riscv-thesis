...
class AccumulatorExampleModuleImp(outer: AccumulatorExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  when (cmd.fire() && (doWrite || doAccum)) {
    printf(p"[once] called with address $addr and funct $funct\n")
    regfile(addr) := wdata
  }
  ...
  // ean perimenei o core apotelesma
  val doResp = cmd.bits.inst.xd
 
  val stallReg = busy(addr)
  //ean thelw na kanw laod alla to interface req den einai etoimo na dexthei 
  val stallLoad = doLoad && !io.mem.req.ready
  // otan prepei na steilw apantisi alla to resp interface den einai etoimo 
  val stallResp = doResp && !io.resp.ready
  
  // o accel mporei na dexthei entoli 
